...
text = ci.read_text()
identity_step = '''      - name: Verify application identity
        run: |
          set -euo pipefail
          ...
          test "$namespace" = "com.sl.impulse"
          test "$application_id" = "com.sl.impulse"
          legacy_id="io.github.stanleyll0yd"".impulse"
          if git grep -nF "$legacy_id"; then
            echo "Legacy application identity remains in the repository." >&2
            exit 1
          fi

'''
needle = '      - name: Set up Android build environment\n'
if '      - name: Verify application identity\n' not in text:
    text = text.replace(needle, identity_step + needle, 1)
ci.write_text(text)

version_code_line = '          version_code="$(sed -n \'s/^[[:space:]]*versionCode = \\([0-9][0-9]*\\).*/\\1/p\' app/build.gradle.kts | head -1)"\n'
identity_lines = '''          namespace="$(sed -n 's/^[[:space:]]*namespace = "\\([^"]*\\)".*/\\1/p' app/build.gradle.kts | head -1)"
          application_id="$(sed -n 's/^[[:space:]]*applicationId = "\\([^"]*\\)".*/\\1/p' app/build.gradle.kts | head -1)"
          test "$namespace" = "com.sl.impulse"
          test "$application_id" = "com.sl.impulse"
'''

tag_workflow = root / '.github/workflows/tag-release-version.yml'
text = tag_workflow.read_text()
if '          application_id="$(sed -n' not in text:
    text = text.replace(version_code_line, version_code_line + identity_lines, 1)
...
if '          application_id="$(sed -n' not in text:
    text = text.replace(version_code_line, version_code_line + identity_lines, 1)
package_guard = '''          aapt="$(find "$ANDROID_HOME/build-tools" -type f -name aapt -print | sort -V | tail -1)"
          test -n "$aapt"
          package_name="$("$aapt" dump badging "$source_apk" | sed -n "s/^package: name='\\([^']*\\)'.*/\\1/p" | head -1)"
          test "$package_name" = "com.sl.impulse"

'''
package_marker = '          test -s "$source_aab"\n\n'
if '          package_name="$(' not in text:
    text = text.replace(package_marker, package_marker + package_guard, 1)
...
for name in (
    ...
    '.github/workflows/prepare-0.3.1.yml',
    'tools/prepare_031.py',
):
    path = root / name
    if path.exists():
        path.unlink()

legacy = ('io.github.stanleyll0yd' + '.impulse').encode()
for path in root.rglob('*'):
    if not path.is_file() or '.git' in path.parts:
        continue
    try:
        ...
        continue
    if legacy in data:
        raise SystemExit(f'Legacy application identity remains in {path}')
