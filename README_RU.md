...
Текущая версия исходного кода: **0.3.1** (`versionCode 4`) · Min SDK: **26 (Android 8.0)** · Target SDK: **37**
...
- Внутриигровые переключатели звука, вибрации и уменьшенных эффектов
- Отдельное сообщение для near miss и усиленная панель результата/повтора
- Кинематографичный запуск: из полностью чёрного экрана изображение IMPULSE проявляется за три секунды
...
- **запуск** — приложение начинается с абсолютного чёрного экрана и за три секунды проявляет изображение IMPULSE;
...
- **результат** — победа, поражение и почти успешная попытка быстро завершаются, поэтому до следующего запуска цепи всегда один шаг.
...
Актуальный публичный прототип **IMPULSE 0.3.1** доступен в [GitHub Releases](https://github.com/StanleyLl0yd/impulse/releases/tag/v0.3.1): опубликованы подписанные APK и AAB, а также SHA-256 checksums.
...
- **Сейчас:** one-tap прототип, детерминированная симуляция, визуальная/звуковая/тактильная полировка, UX результата и кинематографичный запуск
- **Далее:** data-driven уровни, прогрессия, scoring и локальное сохранение состояния
- **Контент:** около 60 подготовленных/генерируемых уровней для первой полноценной версии
- **Позже:** tutorial, accessibility pass, локальная статистика, endless mode и daily challenge после проверки основного цикла уровней
